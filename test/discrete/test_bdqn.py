...
from tianshou.policy import BootstrappedDQNPolicy
...
def get_args():
    ...
    parser.add_argument('--ensemble-num', type=int, default=4)
...
def test_bdqn(args=get_args()):
    ...
    def last_linear(x, y, device):
        return EnsembleLinear(x, y, device, ensemble_num=args.ensemble_num, ensemble_sizes=args.ensemble_sizes)
    # model
    net = NewNet(
        args.state_shape,
        args.action_shape,
        ...
        device=args.device,
        ensemble_param=({ "linear_layer": last_linear})
        # dueling=(Q_param, V_param),
    ).to(args.device)
    optim = torch.optim.Adam(net.parameters(), lr=args.lr)
    policy = BootstrappedDQNPolicy(
        net,
        optim,
        args.gamma,
        args.n_step,
    ...
    # collector
    train_collector = Collector(policy, train_envs, buf, exploration_noise=True, ensemble_num=args.ensemble_num)
